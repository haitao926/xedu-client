...
if xedu_flow_body17 is None:
  raise RuntimeError('当前工作区包含本地不支持的 XEdu 任务，无法执行')
xedu_video_stream = xrt.XEduCamera.video('demo.mp4', window_name="人体关键点 17 Large 视频流")
try:
  print("视频流已启动: 人体关键点 17 Large 视频流")
  while xedu_video_stream.is_opened():
    xedu_stream_frame = xedu_video_stream.read()
    if xedu_stream_frame is None:
      break
    xedu_stream_value = xedu_flow_body17.inference(data=xedu_stream_frame)
...
xrt.xedu_show_result_card(lab_result, title="人体关键点 17 Large 视频 视频")
