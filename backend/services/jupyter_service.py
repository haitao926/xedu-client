...
class JupyterManager:
    ...
    def _prepare_environment(self, config: JupyterConfig) -> dict:
        ...
        # 激活虚拟环境（如果配置了激活脚本）
        if config.activate_script and Path(config.activate_script).exists():
            logger.info(f"激活虚拟环境: {config.activate_script}")
            venv_env = self._activate_virtual_environment(config.activate_script)
            if venv_env:
                env.update(venv_env)
