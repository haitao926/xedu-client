...
export function initDragDrop() {
    console.log('[initDragDrop] 初始化拖拽功能');
    const dropZone = document.getElementById('drop-zone');
    console.log('[initDragDrop] 找到 drop-zone:', dropZone);
    if (dropZone) {
        ...
        ['dragenter', 'dragover'].forEach(eventName => {
            dropZone.addEventListener(eventName, (e) => {
                console.log(`[Drag] ${eventName} 事件触发`);
        ...
        function handleDrop(e) {
            ...
            if (files.length > 0) {
                const file = files[0];
                // 如果有 electronAPI，使用它来获取路径（如果支持）
                // 否则只能获取文件名，或者依赖 Electron 的 webUtils (如果启用)
                // 这里假设用户拖入的是文件夹，且我们能获取 path (Electron 环境下 File 对象有 path 属性)
                if (file.path) {
                    const projectPathInput = document.getElementById('project-path');
                    if (projectPathInput) {
                        projectPathInput.value = file.path;
                        log(`检测到拖拽路径: ${file.path}`, 'info');
                    }
                } else {
                    log('检测到拖拽文件，但无法获取完整路径', 'warning');
                }
            }
        }
    }
