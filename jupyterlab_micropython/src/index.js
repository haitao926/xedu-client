...
function currentFile(app) {
  ...
  return path.toLowerCase().endsWith('.py') ? path : '';
}
...
class MicroPythonPanel extends Panel {
    ...
    this.node.innerHTML = `
      <div class="xedu-micropython-content">
        ...
        <section class="xedu-micropython-section xedu-micropython-console-section">
          ...
          <pre class="xedu-micropython-console" data-role="output">连接后，ESP32 的输出会显示在这里。</pre>
          <div class="xedu-micropython-row">
        ...
        <div class="xedu-micropython-hint">提示：设备需要预先刷入 MicroPython 固件，并使用 USB 数据线连接。</div>
      </div>`;
    this.bindActions();
  ...
  async handleAction(action) {
    try {
      ...
      if (action === 'interrupt') return await request('interrupt', { method: 'POST', body: '{}' });
      if (action === 'reset') return await request('reset', { method: 'POST', body: '{}' });
      if (action === 'send') return await this.sendInput();
    } catch (error) {
      this.setStatus(error.message, true);
      void showErrorMessage('ESP32 操作失败', error.message);
    }
  }
  ...
  async run() {
    ...
    const file = currentFile(this.app);
  ...
  updateFileLabel() {
    this.node.querySelector('[data-role="file"]').textContent = `当前文件：${currentFile(this.app) || '未选择 .py 文件'}`;
  }
    ...
    const poll = async () => {
      try {
        ...
        if (payload.output) {
          ...
          output.textContent = `${output.textContent === '连接后，ESP32 的输出会显示在这里。' ? '' : output.textContent}${payload.output}`;
...
const plugin = {
  ...
  activate: (app, launcher) => {
    ...
    const openPanel = () => {
      ...
      void panel.refreshPorts().catch((error) => panel.setStatus(error.message, true));
    };
