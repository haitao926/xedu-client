...
| Source commit | `89cdbd95` implementation baseline; this local candidate predates the RC2 identity tag |
| Exact release tag | Not applicable to this local candidate; release target is `v2.0.0-rc.2` |
...
- The candidate was not rebuilt from `v2.0.0-rc.2`; its checks are content evidence only and must be repeated on the signed artifacts produced by the official workflow.
