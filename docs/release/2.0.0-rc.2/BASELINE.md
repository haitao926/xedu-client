# XEdu Client 2.0.0 RC.2 Baseline
...
| Release candidate source commit | The commit pointed to by `v2.0.0-rc.2`; implementation baseline `89cdbd95`, followed by the protected-input workflow fix `22d448af` |
| Exact release tag | `v2.0.0-rc.2` (created after the RC2 identity documentation and release-contract update) |
...
| Required release tag | `v2.0.0-rc.2` for this candidate, or a new RC after any source change |
...
- Release workflow contract: any exact `v2.0.0` tag or `v2.0.0-rc.N` tag is accepted only when it points to the checked-out source commit; checkpoint credentials are validated before provisioning and missing prerequisite evidence fails clearly.
...
- The RC2 source commit and `v2.0.0-rc.2` tag are frozen for validation; this does not authorize formal teacher distribution.
