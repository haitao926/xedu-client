...
建议使用独立虚拟环境。至少确保所选环境可以安装项目的 Jupyter、`xedu-python` 和课程所需依赖；不要把 Python 环境复制到应用安装目录。
...
如果首次启动提示选择 Python，打开“Python”设置并选择本机解释器。Windows 选择虚拟环境中的 `Scripts\\python.exe`，macOS 选择虚拟环境中的 `bin/python3` 或 `bin/python`。检测通过后点击“保存设置”。
...
如果首次启动提示选择 Python，打开“Python”设置，选择虚拟环境中的 `bin/python3` 或 `bin/python`。检测通过后点击“保存设置”。
