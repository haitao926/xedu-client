...
> 状态说明：源码整改与本地质量门禁已完成复核；RC5 发布候选已由 `v2.0.0-rc.5` 标识，基线实现为 `89cdbd95`，但现有 `dist-*` 仍是本地诊断产物。只有从该 tag 重新构建、签名并完成实机验收的内容才算正式关闭。
...
RC5 已进入提交和 tag，但本地 `dist-*` 安装包不能替代 tag 产物。团队必须从 `v2.0.0-rc.5` 重新构建、签名并验收最终安装包。
...
| 正式 release workflow 与模型预检 | RC4 官方运行 `29668273546` 已验证 tag/commit 校验通过，并在统一凭据预检中准确列出 9 个未配置 secret；RC5 增加了 Draft/Prerelease GitHub Release 发布步骤，待凭据配置后验证 |
...
- 没有从 `v2.0.0-rc.5` 重新生成并验收完整 Windows 和 macOS 签名安装包。
...
- RC4 官方 release workflow 已执行，但因 9 个受保护 secrets 尚未配置，没有生成候选版本的依赖审计原始 JSON、签名输出或 notarization evidence artifact；RC5 还会在双平台成功后创建 Draft/Prerelease GitHub Release，配置完成后应运行 `v2.0.0-rc.5`。
